...
let f = 0; const total = Math.max(12, target.length * 3);
...
b.className = 'hdot'; b.setAttribute('aria-label', 'اسلاید ' + faNum(i + 1));
b.addEventListener('click', () => { heroIdx = i - 1; nextHero(true); });
...
setTimeout(() => [...document.querySelectorAll('.hbar i')].forEach(b => b.style.width = b.dataset.w + '%'), 600);

const reversedItems = [...items].reverse();
$('#filmTrack').innerHTML = [...reversedItems, ...reversedItems].map((it, i) => `
<figure class="film-frame">
 <img src="${thumbURL(it)}" loading="lazy" alt=""
onerror="this.onerror=null;this.src='${thumbPlaceholder(it, i)}'">
 <b>${faNum((i % TOTAL) + 1)}</b>
</figure>`).join('');
...
const preview = $('#catPreview'), previewImg = preview.querySelector('img');
...
preview.classList.add('show'); previewReady = true;
});
...
/* ───────── بازگشت به بالای صفحه با کلیک روی لوگو ───────── */
...
if (scrollToTopBtn) {
    scrollToTopBtn.addEventListener('click', (e) => {
        ...
        window.scrollTo({
            top: 0,
            behavior: 'smooth' // حرکت نرم و روان به بالای صفحه
        });
    });
}
...
function closeLB(){
lb.classList.remove('open','zoomed'); lb.setAttribute('aria-hidden','true');
...
lbLoader.classList.add('on'); lbNote.hidden = true;
lbChip.textContent = it.cat.fa; lbChip.style.setProperty('--acc', it.cat.color);
...
const d = e.target.closest('.ld'); if (!d) return; idx = +d.dataset.k; showLB();
});
...
try { $('#footYear').textContent = new Date().toLocaleDateString('fa-IR-u-ca-persian', {year:'numeric'}); } catch(e){}
